...
Backend: the browser calls the same-origin `/api` base URL by default. Vite proxies `/api` to `http://127.0.0.1:8000` for local development, avoiding browser-to-backend port/CORS issues. Override with `VITE_API_BASE_URL` when a direct backend URL is needed, and `VITE_API_PROXY_TARGET` when the local API runs elsewhere. Ingestion posts to `/repositories/ingest` before research, then direct RAG posts to `/rag` and agentic RAG posts to `/research`. Ingestion request fields: repository_address string, accepting a local path or public GitHub URL. Direct RAG request fields: question string, mode one of auto/locate/flow/change, retrieval_mode one of dense/sparse/hybrid, limit 1-20, optional repository_path. Agentic RAG uses retrieval_limit instead of limit.
...
Research view controls: API connection status and base URL input, repository address input, ingest button with loading state, question textarea, suggested reviewer questions, direct RAG / agentic RAG segmented control, mode segmented control, retrieval mode segmented control, limit slider, submit button with loading state. The Backoffice route keeps the denser technical control surface.
